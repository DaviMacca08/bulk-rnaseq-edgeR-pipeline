...
- Disease-Free Survival analysis on independent GEO breast cancer
  cohorts
...
- Identification of statistically significant differentially expressed
  genes (DEGs) associated with breast cancer progression  

- Clear transcriptomic separation between experimental groups observed
  through Principal Component Analysis (PCA)  

- Over-representation analysis (ORA) revealed activation of
  extracellular matrix remodeling, hypoxia response, metabolic
  reprogramming, and tumor microenvironment-related pathways  

- Gene Set Enrichment Analysis (GSEA) confirmed coordinated enrichment
  of key biological programs, including HIF-1 signaling,
  glycolysis/gluconeogenesis, and oxidative phosphorylation, indicating
  systemic metabolic reprogramming in breast cancer cells  

- Network-based analyses identified biologically relevant hub genes
  involved in tumor progression and cellular communication  

- Disease-Free Survival analysis using the independent GEO cohort
  GSE2034 identified relapse-associated hub genes linked to breast
  cancer recurrence  

- Overall Survival analysis using TCGA-BRCA RNA-seq and clinical data
  revealed survival-associated candidate prognostic biomarker  

- Multivariate Cox proportional hazards modeling suggested that selected
  hub genes retained prognostic relevance after adjustment for clinical
  covariates
...
1.  Differential expression analysis using edgeR (TMM normalization with
    generalized linear modeling framework)
2.  Functional enrichment analysis: Over-representation analysis (Gene
    Ontology, KEGG, Reactome) and Gene Set Enrichment Analysis (GSEA)
    for pathway-level interpretation
3.  Exploratory data visualization (PCA, hierarchical clustering,
    heatmaps, volcano plots)

### Systems biology and network analysis

4.  Weighted Gene Co-expression Network Analysis (WGCNA) for module
    detection and gene clustering
5.  Protein–Protein Interaction (PPI) network construction and
    visualization
6.  Network topology analysis and hub gene prioritization using
...
7.  Survival analysis for clinical relevance assessment:
    ...
    - Overall Survival analysis using TCGA-BRCA clinical data
    - Kaplan–Meier survival modeling and Cox proportional hazards
      regression
...
- **GO Over-Representation Analysis (ORA): significant Biological
  Process (BP), Molecular Function (MF), and Cellular Component (CC)
  terms**  

![](downstream/plots/ORA/01-GO_enrichment.png)

- **GSEA: top contributing genes in HIF-1 signaling,
...
- **Kaplan–Meier overall survival stratification based on PGK1
  expression levels**
...
These transcriptional programs are consistent with survival-associated
molecular signatures observed in breast cancer, where genes involved in
extracellular matrix remodeling, metabolic reprogramming, and pro-growth
signaling have been linked to both disease recurrence and reduced
overall survival in independent patient cohorts.
...
3.  **TCGA-BRCA dataset:** The Cancer Genome Atlas Breast Cancer cohort
    (TCGA-BRCA), accessed via the National Cancer Institute GDC Portal –
    <https://www.cancer.gov/ccg/research/genome-sequencing/tcga>
    (accessed on 10 May 2026)  
4.  **BioProject:**
    [PRJNA1161137](https://www.ncbi.nlm.nih.gov/bioproject/PRJNA1161137)  
5.  **RNA-seq best practices:** [Bioconductor RNA-seq
...
2.  **Bioconductor / CRAN packages:** edgeR (v4.8.2), WGCNA (v1.74),
    ...
    survival (v3.8.6), survminer (v0.5.2)
3.  **PPI tools:**
